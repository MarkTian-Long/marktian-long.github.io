...
- **新增文章**：遵循 `WRITING_GUIDE.md` 和 `VISUAL_GUIDE.md`。最终 Markdown 的标题下唯一 blockquote 是正式 `summary`，发布阶段原样同步到 `data/posts-meta.json`；Codex 只从终稿正文选择 `share_quote`，Markdown 无需携带发布 metadata；仅当内容评审已确认强关系时，额外维护可选 `relations`。每篇新文章必须有一张单篇封面；正文只在能明显降低理解成本时配 0–2 张图
...
- **发布生成**：先在 `posts-meta.json` 同步 Markdown blockquote 得到的 `summary` 并添加其他元数据；用 `scripts/prepare-blog-image.js` 生成规范封面（及按需的正文图），再维护 `visuals` 和 Markdown 独立图片语法。随后运行 `node tools/blog/generate-post.js --write <source.md> <output.html>`；生成器会拒绝 summary 或图片声明与最终 Markdown 不一致的源稿，最后运行 `node scripts/generate-search-assets.js --write`
