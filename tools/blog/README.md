...
- **taxonomy impact review**：变更 taxonomy 的新增、定义/范围调整、rename、merge 或 deprecate 时，先运行确定性校验，再执行 `node tools/blog/audit-taxonomy-impact.js --base-ref <变更前-ref>`（或在已落地定义的专项复核中显式 `--focus category:<名称>`）。定义/范围调整会保守地让全部历史 metadata 进入人工筛查，`--categories` 只标示优先阅读范围，避免跨分类文章因未出现新词而被静默漏掉。脚本不改历史 metadata；Codex 按“线上正式页 → 仓库 HTML → Markdown”读真正相关正文，逐篇给出保持/人工复核/有充分理由建议迁移，不为分类数量均衡迁移。
