...
- **新增文章**：遵循 `WRITING_GUIDE.md` 和 `VISUAL_GUIDE.md`。最终 Markdown 的标题下唯一 blockquote 是正式 `summary`，发布阶段原样同步到 `data/posts-meta.json`；Codex 只从终稿正文选择 `share_quote`，Markdown 无需携带发布 metadata；仅当内容评审已确认强关系时，额外维护可选 `relations`。封面、正文图和纯文字文章都可按内容需要选择；无图文章的分享元数据回退站点默认图
...
- **发布生成**：先在 `posts-meta.json` 同步 Markdown blockquote 得到的 `summary` 并添加其他元数据；若终稿末尾含 transport-only `publish_handoff`，先运行 `node tools/blog/publish-handoff.js --write <source.md>`，由它校验、同步 `relations` 并剥离交接块。图片完全可选；如使用图片，再用 `scripts/prepare-blog-image.js` 处理成品并维护 `visuals` 和 Markdown 独立图片语法，随后运行 `node tools/blog/generate-post.js --write <source.md> <output.html>`；生成器会拒绝未处理的交接块，以及 summary 或已声明图片与最终 Markdown 不一致的源稿，最后运行 `node scripts/generate-search-assets.js --write`
