...
- **新增文章**：遵循 `WRITING_GUIDE.md` 和 `VISUAL_GUIDE.md`。最终 Markdown 的标题下唯一 blockquote 是正式 `summary`，发布阶段原样同步到 `data/posts-meta.json`；Codex 只从终稿正文选择 `share_quote`，Markdown 无需携带发布 metadata；仅当内容评审已确认强关系时，额外维护可选 `relations`。封面只在能提供不同于正文图的独立解释价值时添加；无封面时需保留至少一张确有解释价值的正文图，OG 回退站点默认图
- **文章清单**：以 `data/posts-meta.json` v4 为单一来源；根级 `image_contract` 管理历史无图豁免，单篇 `visuals` 声明封面和正文图。主页与 Blog 列表仍保持纯文字，不回填历史文章图片；上方目录只保留近期与代表性文章，避免手工清单漂移
...
- **发布生成**：先在 `posts-meta.json` 同步 Markdown blockquote 得到的 `summary` 并添加其他元数据；若终稿末尾含 transport-only `publish_handoff`，先运行 `node tools/blog/publish-handoff.js --write <source.md>`，由它校验、同步 `relations` 并剥离交接块。再用 `scripts/prepare-blog-image.js` 生成规范封面（及按需的正文图），维护 `visuals` 和 Markdown 独立图片语法，随后运行 `node tools/blog/generate-post.js --write <source.md> <output.html>`；生成器会拒绝未处理的交接块，以及 summary 或图片声明与最终 Markdown 不一致的源稿，最后运行 `node scripts/generate-search-assets.js --write`
