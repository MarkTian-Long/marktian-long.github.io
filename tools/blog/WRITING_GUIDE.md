...
| 标签 | 文章在做什么 | 区分说明 |
|------|------------|---------|
| 竞争判断 | 分析商业格局、产品竞争、并购信号 | 聚焦单一事件/产品的竞争含义 |
| 市场格局 | 行业整体走向、开放/封闭博弈、生态演变 | 视角更宏观，跨多方势力 |
| 技术判断 | 判断某项技术的生命周期、价值与边界 | 结论是「这个技术值不值得做/还有多久消亡」 |
...
**注意：**
- callout 必须是文章 `post-body` 内的**最后一个元素**（双栏长文见「双栏布局规范 → callout 位置规则」，该规则优先）
