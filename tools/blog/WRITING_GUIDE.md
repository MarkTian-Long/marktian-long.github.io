...
写新文章前，先用 `title`、`summary`、`concepts`、`topics`、`tags` 和 `category` 从历史元数据中找少量候选，再读正文确认。优先读 `docs/blog/<slug>.md`；历史文章没有同名 Markdown 时直接读 `tools/blog/posts/<slug>.html`。元数据命中只意味着“值得打开”，不构成引用理由：只有问题、机制、观点延续/修正、框架、证据或读者理解所需的连续性确实成立时才引用。
...
| 产品 | PM 决策框架、产品分析、场景判断、功能设计；含具体 AI 产品/工具的拆解与竞争信号分析（核心落点是产品决策，非行业格局） |
...
- `tools/blog/data/posts-meta.json` 仍是标题、摘要、标签、检索概念、分类和 URL 的单一来源；Markdown 负责正文，HTML 是部署产物。历史正文检索时，缺少同名 Markdown 可回退读取当前发布 HTML，不需要为此批量补写 Markdown。
...
- 写作或引用旧文前按本指南的两阶段检索流程处理：先从元数据召回候选，再读正文判断实质关联；不要因为同关键词或同分类强行添加内链。
...
- 检查相关文章：按当前推荐评分策略查看前两篇推荐，确认它们与文章的核心问题相关；若因某个 `topic` 产生明显偏题的推荐，应重新审视该标签。
