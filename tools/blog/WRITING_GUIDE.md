...
本文件定义「思考碎片」博客的元数据、结构、命名和更新规范，所有文章必须遵守。博客研究与写作流程从 `governance/README.md` 进入；本文件只维护最终 Markdown 与网站发布契约。

未发布的完整工作稿可以原位维护在仓库根目录 `drafts/blog/<slug>.md`，以便跨会话共用 GitHub 单一当前稿且不触发 `docs/blog/` 的发布源校验。用户明确确认发布后，才将当前稿迁移到 `docs/blog/<slug>.md`，同步 metadata、生成 HTML，并删除对应 draft，避免双版本。
...
- 不写 `AI`、`技术`、`产品`、`商业`、`行业`、`实践` 等几乎没有区分度的泛词；不要因正文偶然提到品牌或术语就升级为 concept。
...
>
> **「实践」不是第一人称文章的收纳箱。** 只有当真实工作、项目、使用或决策经历本身构成文章主要研究对象和证据链，并从中沉淀方法、边界或判断时，才归「实践」。个人经历只作为引子时，仍按技术 / 产品 / 商业 / 行业的核心对象分类；教程也不会仅因可操作而自动归「实践」。
