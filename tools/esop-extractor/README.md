...
## 核心设计亮点
- **金融同义词映射**：10 类术语变体提取为独立数据结构 `SYNONYM_MAP`，左侧面板可视化展示，同时注入 Prompt 全局复用
- **章节定位提示**：Prompt 中预标注各字段所在招股书章节，引导模型定位
- **缺失字段兜底**：找不到时返回 null，不编造内容
- **溯源三元素**：每个字段附带原文来源片段，支持核查；结果头部实时显示溯源覆盖率
- **质量指标看板**：结果头部展示高置信度率 + 溯源覆盖率，对应 POC 验收标准（≥95% / 100%）
- **字段间逻辑校验**：提取完成后自动执行跨字段规则检查（授予超池、行权价异常、授予日期早于计划等），异常字段显示黄色 ⚠ 警告及原因
