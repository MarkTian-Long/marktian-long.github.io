...
热点快照 v2 是一个可追溯的 AI 信号研判台。它把五个平台的榜单记录成带来源、观察时间、指标口径、行动分类和编辑判断的静态快照，不把静态数据伪装成实时热度。
...
- `review_scope=structure_only` 必须同时使用 `collection_mode=structure_checked`、`verification_level=structure_checked`，表示只完成结构与编辑字段检查，`facts_verified_at` 必须为 `null`；条目也必须是 `structure_checked`。
- `review_scope=facts_verified` 必须同时使用 `collection_mode=manual_fact_reviewed`、`verification_level=manual_fact_reviewed`，且每条信号都逐条完成事实核验并填写 `facts_verified_at`；只有这一状态可以进入公开写入。
- 候选数据使用 `collection_mode=candidate`、`verification_level=candidate`、`review_scope=candidate`，只能进入结构复核队列，不能直接发布。

`manual_fact_reviewed` 只代表存在逐条事实核验日期，不能用作结构检查的别名；旧的 `manual_reviewed` 状态不再是合法 v2 状态。当前历史快照是 `structure_checked` / `structure_only`，不是本轮事实核验结果。
...
完成候选的结构检查，并逐条复核来源、时间、指标、行动和判断事实后，才允许写入 `tools/trends/data/`。公开写入必须使用 `review_scope=facts_verified`、`collection_mode=manual_fact_reviewed`、`verification_level=manual_fact_reviewed` 和非空 `facts_verified_at`；仅结构检查的快照会被写入门禁拒绝。写入前会再次验证完整 v2 契约，写入命令不联网：

```bash
node fetch-trends.js --write --input ..\build\candidate-site\trends-reviewed.json
```

如需测试其他公开目标，`--target` 也只能指向 `tools/trends/data/` 下的文件。候选目录或仓库外路径会被拒绝。写入前先完成路径校验、JSON 校验和事实复核状态校验，失败不会创建目标文件。
...
仓库内当前公开快照的 `as_of` / `observed_at` 是 `2026-05-19`，页面在 `2026-08-30` 显示“历史快照，不代表当前热度”。页面将其标成“本轮仅结构检查；历史事实未重验”：`reviewed_at=2026-08-30` 只表示契约和结构字段复核日期，`review_scope=structure_only`、`collection_mode=structure_checked`、`verification_level=structure_checked`、`facts_verified_at=null` 不表示事实已人工核验。历史记录中的数值、排序、事实和因果判断不能当作 2026-08-30 的当前事实。
...
├── data/
│   └── trends.json   # 公开快照；当前为仅结构检查的历史观察记录
└── README.md
```
...
浏览器测试使用固定时钟覆盖 `2026-08-30` 正常交互，以及快照观察日后的第 7、8、30、31 天状态；同时覆盖筛选、键盘、来源、移动端、空板块、404 和无效 JSON 状态。启动器优先使用 `PLAYWRIGHT_EXECUTABLE_PATH`，未设置时在 Windows 本机 Edge 路径中寻找 `msedge.exe`，最后才回退到 Playwright bundled Chromium。
