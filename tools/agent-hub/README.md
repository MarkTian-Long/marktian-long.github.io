...
- **风险控制**：高风险或不可逆动作强制预览、HITL、审计和停止条件；输入缺失或矛盾时撤回自动建议。
- **架构与故障**：单 Agent + 工具、主从并行、互审 / 辩论、显式工作流，分别说明故障传播、重试边界和人工接管。
- **六个场景预设**：招聘初筛、代码审查、客服工单、BI 周报、法务文档、IT 帮助台；每个场景都有输入假设、人的责任、测量方式和停止条件。
- **框架事实**：LangGraph、OpenAI Agents SDK、CrewAI、Microsoft Agent Framework、Dify、OpenAI Swarm 均附官方来源、核对日期和 90 天复核周期。超过 90 天进入“待复核”，超过 180 天显示“过期资料”并撤回当前推荐。
- **判断框架**：六条带适用条件、判断规则、反例和下一条证据需求的产品判断。

## 可信边界

页面是静态规则判断器，不调用模型、不接入生产数据，也不保存问卷答案。场景中的 target、proxy 和 offline-measured 指标是目标、代理指标或离线测量设计，不是生产结果或效果承诺。

框架事实在 2026-08-30 按官方文档核对。产品状态、能力和链接会变化，使用前请依照卡片上的复核日期重新确认。
...
├── data/
│   └── decision-model.js      # 六问、规则、架构、场景、判断与官方事实
└── README.md
~~~
