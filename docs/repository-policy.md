...
| Claude 兼容 Skill | `.claude/skills/` | 为 Claude 提供兼容入口；项目自定义 Skill 必须由 `.agents/skills/` 单向同步提交，不能独立演化；运行 `node scripts/check-project-skill-compatibility.js` 检查全目录漂移 |
