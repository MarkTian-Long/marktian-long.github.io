...
[FARS](https://analemma.ai/blog/introducing-fars/)是一个端到端的自动化科研系统，由四个专职Agent组成：Ideation负责文献调研和假设生成，Planning制定实验方案，Experiment执行实验，Writing完成论文写作。整个流程无需人工干预，相关代码完全开源在GitLab。
...
有几点边界需要说清楚：FARS目前主要针对机器学习领域的研究，它能全流程自动化的前提是实验可以在代码层面完成，不需要湿实验室。生物、化学、材料科学等依赖物理实验的领域，暂时无法复制这套模式。
...
**假设生成型工具**往前迈了一步，开始介入研究方向的提议环节。[Google AI co-scientist](https://research.google/blog/accelerating-scientific-breakthroughs-with-an-ai-co-scientist/)基于Gemini 2.0构建，通过多个专职Agent持续生成、辩论和改进研究假设。它已在生物医学领域得到实际验证：与Imperial College London合作的抗菌素耐药性研究中，系统在数天内得出了传统方式需要数年才能推进的结论；在急性髓系白血病的药物重定向研究中，它提出的候选药物经湿实验室验证得到证实。Google明确将其定位为辅助工具，强调需要"科学家持续介入和引导"。
...
这就触碰到了AI目前面临的真正瓶颈：**价值判断能力**，也就是从大量可行的方向中，识别出哪些真正重要。

这个能力的缺口，可以拆成两层来看。

第一层是**上下文缺口**：AI因为缺乏对具体场景的深度理解，导致判断偏离实际。这层问题理论上有解，通过补充私有数据、细化场景描述、构建更完整的知识库可以缓解，是个工程问题。

第二层是更根本的**方向判断缺口**：即使信息足够，AI仍然无法判断"这个方向在更大图景里重不重要"。一项专门研究AI与人类在科研选题上分歧的[实验发现](https://arxiv.org/pdf/2602.03849)：AI在识别"已有突破性成果"方面与人类专家高度一致，但在预测"前瞻性研究方向"时出现明显分歧。这层能力依赖的是对领域走向的感知、对未来的预判，以及对"什么是有意义的问题"的价值取向，很难从现有语料里学到。

## 价值判断本来就是困难的事

价值判断本来就是困难的事，对人类也不例外。
...
这些案例说明的不是"同行评审很烂"，而是：在一个方向被验证之前，判断它的价值几乎是不可能完成的任务。人类也经常判断错。把这个逻辑带回FARS的5.05平均分：我们没有办法仅凭学术评审标准，判断其中有多少篇论文指向了真正重要的方向——就像没有人能在1986年用当时的评审标准判断Hinton的研究会通向哪里。
...
在科研领域，这意味着AI大规模生成候选方向和执行实验，人类专家在关键节点做筛选。已经有[研究团队在构建更系统化的协作框架](https://arxiv.org/pdf/2602.03849)，让AI处理文献和假设生成，人类在价值判断环节介入。Google AI co-scientist也是这个逻辑，它明确强调需要"科学家持续介入和引导"。
...
- [HybridQuestion: Human-AI Collaboration for Identifying High-Impact Research Questions](https://arxiv.org/pdf/2602.03849)
