...
|---|---|---|---|
| 博客生成器 | 截至 2026-08-19，`main` 已改用受控的 `tools/blog/article-template.html`，但 `generate-post.js` 仍直接写调用方指定的输出路径且没有 `--check` | 模板耦合已解除；误传公开路径时仍可能直接覆盖文章产物 | 保留唯一受控模板；增加默认只读的 `--check` 和输出路径白名单，再逐篇、经确认迁移 |
...
1. 博客已具备独立、版本控制的模板；下一步用受控 Markdown fixture 生成内存结果，增加只比较、不写 HTML 的 `--check`，并限制写入目标。禁止批量重生历史文章。
