...
| 生成器可以依赖已发布 HTML 或直接覆盖公开产物；趋势局部失败可写空 board | Track A 初始 4 个信号；2026-08-19 集成时确认博客已改用受控模板，checker 将旧耦合信号替换为仍存在的“无 `--check` 直接输出”信号 | 3 个独立生成器均出现 | memory + checker/test + migration plan | `docs/agent-context/memory.md`、`scripts/check-generator-contracts.js`、`docs/plans/2026-07-31-generator-safe-migration.md` | 报告只读；迁移步骤要求 `--check`、candidate 和显式写入 | 部分缓解 | Track B 后续逐个产物迁移 |
