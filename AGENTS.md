...
## 跨 Agent 共享上下文（必须先读）
本项目同时使用 Claude Code 与 Codex 维护。任何 Agent 开始修改或审查本项目之前，必须先按顺序读取：
...
项目自定义 skill 清单维护在 `scripts/repository-policy.json`，正文统一维护在 `.agents/skills/<name>/SKILL.md` 并纳入 Git。`.claude/skills/` 是 Claude 兼容层，其中项目自定义 skill 必须与 `.agents/skills/` 文件名和内容一致；第三方设计 skill 由 `skills-lock.json` 管理，本机 `.agents/skills/` 安装产物不提交。Agent 专属说明写在 AGENTS.md / CLAUDE.md，不分叉共享 Skill。不要再新建独立的 `.Codex/skills/` 作为项目规范源。
