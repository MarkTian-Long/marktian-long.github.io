...
在终端运行以下命令（在项目内临时目录中执行，避免污染用户项目；Windows 下 `/tmp` 不可用）：

```bash
mkdir -p tools/.design-tmp && cd tools/.design-tmp && npx getdesign@latest add <slug> 2>&1
```

然后读取生成的 DESIGN.md 文件：

```bash
cat tools/.design-tmp/DESIGN.md
```

完成 UI 生成后，清理临时目录：

```bash
rm -rf tools/.design-tmp
```
